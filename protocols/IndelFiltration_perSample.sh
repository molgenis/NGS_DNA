set -o pipefail
#Parameter mapping
#string tmpName
#string gatkVersion
#string tempDir
#string intermediateDir
#string indexFile
#string tmpDataDir
#string project
#string logsDir
#string groupname
#string sampleIndelsOnlyVcf
#string sampleIndelsOnlyFilteredVcf

#Load GATK module
module purge
module load "${gatkVersion}"

makeTmpDir "${sampleIndelsOnlyFilteredVcf}"
tmpSampleIndelsOnlyFilteredVcf="${MC_tmpFile}"

#Run GATK VariantFiltration to filter called Indels on
gatk --java-options "-XX:ParallelGCThreads=1 -Djava.io.tmpdir=${tempDir} -Xmx4g" VariantFiltration \
-R "${indexFile}" \
-O "${tmpSampleIndelsOnlyFilteredVcf}" \
-V "${sampleIndelsOnlyVcf}" \
--filter-name "filterQD" \
--filter-expression "QD < 2.0" \
--filter-name "filterSOR_gt10.0" \
--filter-expression "SOR > 10.0" \
--filter-name "filterReadPosRankSum" \
--filter-expression "ReadPosRankSum < -20.0"

mv -v "${tmpSampleIndelsOnlyFilteredVcf}" "${sampleIndelsOnlyFilteredVcf}"
